...
public:
    static int count;
    // Конструктор для инициализации точек            Определение константного поля
    CircleSector(const double x1, const double y1, const double x2, const double y2): objectId(count++) {
        //x1 = 123; // Вызывает ошибку
        this->x1 = x1;
        this->y1 = y1;
        this->x2 = x2;
        this->y2 = y2;
    ...
    static void setCount(int num) {
        CircleSector::count = num;
    }
    static int getCount() {
        double a = x1;
        double b = this.x1;
        return CircleSector::count;
    }
    ...
    //Константный метод
    void printObjectId() const {
        // Вывод идентификатора объекта
        std::cout << "Object ID: " << this->objectId << std::endl;
    }
    ...
    CircleSector(const CircleSector& other) : objectId(count++)
    {
        this->x1 = other.x1;
        this->x2 = other.x2;
        this->y1 = other.y1;
...
int CircleSector::count = 0;


int main() {
    setlocale(LC_ALL, "ru");

    CircleSector::setCount(10);
    ...
    sector1.printObjectId();
    sector2.printObjectId();
    

    //sector1.setCount(10);
    std::cout << CircleSector::count << std::endl;
